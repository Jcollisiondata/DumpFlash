...
class uImage:
    ...
    def dump_header(self):
        ...
        print('Name:\t%s'% (self.name.rstrip(b'\x00').decode('utf-8', errors='replace')))
...
class Util:
    ...
    def find(self):
        ...
        while True:
            ret = self.FlashImageIO.check_bad_block(block)
...
def build_parser():
    ...
    parser.add_argument('-f', action='store_true', dest='fix_header', default=False)
    parser.add_argument('-c', action='store_true', dest='check_crc', default=False)
    parser.add_argument('-e', action='store_true', dest='extract', default=False)
    parser.add_argument('-m', action='store_true', dest='merge', default=False)
    parser.add_argument('-o', '--output_filename', dest='output_filename', default='', help='Set output_filename filename', metavar='output_filename')
    parser.add_argument('-H', '--header_file', dest='header_file', default='', help='Set header filename', metavar='header')
...
def main(argv=None):
    ...
    if options.fix_header:
        ...
        if not options.header_file:
            parser.error('merge requires --header_file')
        if not options.output_filename:
            parser.error('merge requires --output_filename')
        uimage.merge(options.header_file, options.files, options.output_filename)
