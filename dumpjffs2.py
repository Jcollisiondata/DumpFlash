...
class JFFS:
    ...
    def parse(self, filename, target_filename = ''):
        ...
        target_filename_bytes = _to_bytes(target_filename) if target_filename else b''
...
#            magci_header_offset = data_offset
            ...
            if nodetype == JFFS2_NODETYPE_INODE:
                ...
                if error or (target_filename_bytes and ino in self.DirentMap and self.DirentMap[ino]['payload'].find(target_filename_bytes) >= 0):
                    #if self.DebugLevel>0:
                    if True:
                        print(' = '*79)
                ...
                if target_filename_bytes and payload.find(target_filename_bytes) >= 0:
                    print(' = '*79)
                    ...
                    if pino in self.DirentMap:
                        parent_node = _to_text(self.DirentMap[pino]['payload'])

                    print('Payload:\t%s' % (parent_node + '\\' + _to_text(payload)))
                    print('')
    ...
    def get_path(self, ino):
        ...
        while ino != 0 and ino in self.DirentMap:
            path = '/' + _to_text(self.DirentMap[ino]['payload']) + path
    ...
    def read_file_data(self, inode_map_record, dump = False):
        ...
        for record in inode_map_record:
            ...
            if new_data_len > 0:
                data.extend(b'\x00' * new_data_len)
        ...
        while True:
            ...
            for record in inode_map_record:
                ...
                if offset == next_offset:
                    ...
                    original_csize = record['csize']
                    if original_csize > len(data):
                        ...
                        with open(output_filename, 'r+b') as fd:
                            ...
                            fd.write(struct.pack(inode_unpack_fmt,
                                                 ...
                                                 record['data_crc'],
                                                 record['node_crc']
                                                ) + data + (original_csize - len(data)) * b'\xff')
        ...
        if path == target_filename:
            ...
            for record in self.INodeMap[ino]:
                ...
                if record_offset <= offset and offset <= record_offset+record_dsize:
                    ...
                    with open(new_data_filename, 'rb') as fd:
                    ...
                    if output_filename != '':
                        ...
                        with open(self.OrigFilename, 'rb') as orig_fd:
                            old_data = orig_fd.read()

                        #Save
                        with open('old.bin', 'wb') as ofd:
                            ofd.write(old_data[record_data_offset:record_data_offset+len(new_inode)])

                        with open('new.bin', 'wb') as nfd:
                            nfd.write(new_inode)
...
def build_parser():
    ...
    parser.add_argument('-o', '--output_dir', dest='output_dir', help='Set output directory name', default='', metavar='OUTPUT_DIR')
    parser.add_argument('-O', '--output_filename', dest='output_filename', help='Set output filename', default='', metavar='OUTPUT_FILENAME')
    parser.add_argument('-f', '--target_filename', dest='target_filename', help='Set target filename', default='', metavar='TARGET_FILENAME')
    parser.add_argument('-n', '--new_data_filename', dest='new_data_filename', help='Set new data file name', default='', metavar='NEW_DATA_FILENAME')
    parser.add_argument('-l', action='store_true', dest='list')
    parser.add_argument('-d', type=int, default=0, dest='debug')
    parser.add_argument('-t', type=int, default=0, dest='offset')
    parser.add_argument('-s', type=int, default=0, dest='size')
    parser.add_argument('jffs2_filename')
    return parser
...
def main(argv=None):
    ...
    jffs.parse(options.jffs2_filename, target_filename=options.target_filename)

    if options.list:
        jffs.list_file(options.target_filename)
    ...
    elif options.target_filename and options.output_filename:
